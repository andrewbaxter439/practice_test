...
```{r include=FALSE}
minimal_test_date <- covid_data %>%
  filter(perc_pos == min(perc_pos)) %>%
  select(Date, new_cases, tot_tests) %>%
  pull(Date)
```

What date had the minimal positive test rate? Well, it was `r minimal_test_date`.

```{r echo=FALSE, fig.cap="Graphs of daily test results and numbers."}
...
```{r echo=FALSE}
covid_data %>% 
  ggplot(aes(as.POSIXct(Date), new_cases)) +
  ylab("Number of new cases reported") +
  xlab("Date") +
  theme_minimal() +
...
What is the correlation between test positive rates of hospitalisaitons on day = d and test positive rates on day = d-1?
